#importing the library 'random' to get a random pseudo number, this library cannot be used in password generation project.
import random

#set the maximum number of attempts in the game, lower limit and the upper limit for the random generated number.
lower_limit = 0
upper_limit = 20
max_attempts = 10


#Store the random generated number in the variable 'secret_number'
...
def get_guess():
  while True:
      ...
      except ValueError:      #if the guess number is anything except an 'integer', it is considered as a ValueError.
         print("Invalid Guess. Please put a valid guess number.")

# Checks if the number is equal to, less than or greater than the 'secret number', gives the result accordingly.
...
#this will display everytime we run the code
if __name__ == "__main__":
  ...
  #1) add levels, 2) add levels according to the difficulty, 3) make a ui to make it more interactive, 4) use similar logic to make word guessing game..  